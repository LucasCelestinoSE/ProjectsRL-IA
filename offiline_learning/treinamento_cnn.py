...
class PongPreprocessing(gym.ObservationWrapper):
    def __init__(self, env):
        ...
        self.observation_space = gym.spaces.Box(
            low=0,
            high=255,
            shape=(1, 84, 84),  # Reduzido para 84x84 e um canal
            dtype=np.uint8
        )
    ...
    def observation(self, obs):
        ...
        # Adiciona dimensão do canal e normaliza
...
class MCTSDQNPong:
    def __init__(self, env, model_path=None, exploration_weight=1.0):
        ...
        # Inicializa ou carrega o modelo DQN com buffer size reduzido
        if model_path:
            ...
            self.dqn = DQN(
                ...
                buffer_size=10000,  # Reduzido de 100000 para 10000
    ...
    def get_dqn_action_probs(self, state):
        ...
        with torch.no_grad():
            q_values = self.dqn.q_net(torch.as_tensor(state).float())
    ...
    def choose_action(self, state, num_simulations=50):  # Reduzido número de simulações
        root = Node(state)
    ...
    def simulate(self, node):
        ...
        total_reward = 0
        max_steps = 50  # Reduzido de 100 para 50
    ...
    def train_dqn(self, total_timesteps=100000):  # Reduzido de 10000 para 5000
...
def main():
    ...
    agent.save_dqn("pong_dqn")
    
    # Executa episódios usando a combinação MCTS-DQN
    max_episodes = 5
